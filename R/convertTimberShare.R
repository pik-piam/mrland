...
convertTimberShare <- function(x) {
  ...
  map$Region <- as.character(map$Region) # nolint
  map$ISO3 <- as.character(map$ISO3)     # nolint
  map$ISO2 <- as.character(map$ISO2)     # nolint
  map <- map[!duplicated(map$ISO3), ]
  map <- map[map$Region != "Antarctica", ]
  map <- map[!is.na(map$LETTERCODE), ]
  y <- toolAggregate(x = x, rel = map, from = "Region", to = "ISO3", dim = 1, weight = NULL, partrel = FALSE)
