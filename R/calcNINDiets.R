...
calcNINDiets <- function(attributes = c("wm","kcal"), calib = TRUE, FAOcountr = FALSE){
  ...
  Mag_NIN_diets <- Mag_NIN_diets[,,"dummy", pmatch=T, invert=T]
  ...
  NIN_Lancet_intake_no_balancing <- dimSums(NIN_diets[,,c("othrcrp","othrcal"),invert=T],dim=3.4)
  Mag_NIN_intake_no_balancing <- dimSums(Mag_NIN_diets[,,Mag_sugrcrp,invert=T][,,"alcohol",invert=T],dim=3.4)
