...
calcClimateRegionsIPCC<-function(landusetypes="all", cellular=FALSE, yearly=FALSE, convert=TRUE){
  ...
  if (cellular==TRUE){
    ...
    climate_regions <- toolAggregate(x = climate_regions, rel = CountryToCell, weight = landuse_sum, from = "celliso", to = "iso", partrel = TRUE)
    out <- climate_regions
    
    if (convert==TRUE){
      out <- toolCountryFill(out,fill=0)
      missing <- setdiff(getRegions(out), getRegions(climate_regions))
